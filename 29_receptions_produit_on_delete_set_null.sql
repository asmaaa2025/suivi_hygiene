-- Preserve reception rows when a product is deleted (HACCP traceability).
-- Before delete: copy product name into legacy text columns if still empty.
-- Then FK uses ON DELETE SET NULL so receptions remain with archived name.
...
COMMENT ON FUNCTION public.trg_snapshot_produit_nom_before_produit_delete() IS
  'Copies produit.nom into receptions.produit/article before row delete so history stays readable after SET NULL.';
