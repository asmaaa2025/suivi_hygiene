...
class _TemperaturesListPageState extends State<TemperaturesListPage> {
                        ...
                        : RefreshIndicator(
                            ...
                            child: ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                final isAlert = (appareil.tempMin != null && temp.temperature < appareil.tempMin!) ||
                                    (appareil.tempMax != null && temp.temperature > appareil.tempMax!);
                                ...
                                return SectionCard(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      if (temp.remarque != null && temp.remarque!.isNotEmpty) ...[
                                      ...
                                      if (temp.photoUrl != null && temp.photoUrl!.isNotEmpty) ...[
                                        const SizedBox(height: 8),
                                        Builder(
                                          builder: (context) {
                                            ...
                                            final isValidUrl = storageService.isValidStorageUrl(photoUrl);
                                            final isLocalPath = storageService.isLocalPath(photoUrl);
                                            
                                            if (isLocalPath) {
                                              debugPrint('[TemperaturesList] ⚠️ Photo has local path (not uploaded to Storage): $photoUrl');
                                              debugPrint('[TemperaturesList] This photo needs to be re-uploaded to Supabase Storage.');
                                            } else if (isValidUrl) {
                                              debugPrint('[TemperaturesList] ✅ Valid Supabase Storage URL: $photoUrl');
                                            } else {
                                              debugPrint('[TemperaturesList] ⚠️ Unknown photo URL format: $photoUrl');
                                            }
                                            
                                            return InkWell(
                                              onTap: isValidUrl
                                                  ? () {
                                                      debugPrint('[TemperaturesList] Opening photo: $photoUrl');
                                                  ...
                                                  child: isValidUrl
                                                      ? Image.network(
                                                          ...
                                                          errorBuilder: (context, error, stackTrace) {
                                                            debugPrint('[TemperaturesList] ❌ Error loading photo: $error');
                                                            debugPrint('[TemperaturesList] Photo URL: $photoUrl');
                                                            return const Center(
                                                              child: Column(
                                                                mainAxisSize: MainAxisSize.min,
                                                                children: [
                                                      ...
                                                      : Center(
                                                          child: Column(
                                                            ...
                                                            children: [
                                                              Icon(
                                                                isLocalPath ? Icons.upload_file : Icons.warning,
                                                                size: 40,
                                                                color: Colors.orange,
                                                              ),
                                                              const SizedBox(height: 4),
                                      ...
                                      if (temp.employeeFirstName != null && temp.employeeLastName != null) ...[
                                        const SizedBox(height: 8),
                                        Row(
                                          children: [
                                      ...
                                      Row(
                                        ...
                                        children: [
                                          IconButton(
                                            ...
                                            onPressed: () => _editTemperature(context, temp),
                                            tooltip: 'Modifier',
                                          ),
                                          IconButton(
                                            icon: const Icon(Icons.delete_outline, size: 20),
                                            color: AppTheme.statusCritical,
                                            onPressed: () => _deleteTemperature(context, temp),
                                            tooltip: 'Supprimer',
                                          ),
                                        ],
                                      ),
  ...
  Future<void> _deleteTemperature(BuildContext context, Temperature temp) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Supprimer'),
        content: Text('Êtes-vous sûr de vouloir supprimer cette température (${temp.temperature}°C) ?'),
        actions: [
          TextButton(
