...
class _EmployeeSelectionPageState extends State<EmployeeSelectionPage> {
  ...
  Future<void> _selectEmployee(Employee employee) async {
    ...
    debugPrint('[EmployeeSelection] Selecting employee: ${selectedEmployee.fullName}');
    debugPrint('[EmployeeSelection] Employee ID: ${selectedEmployee.id}');
    debugPrint('[EmployeeSelection] Organization ID: ${selectedEmployee.organizationId}');
    ...
    if (selectedEmployee.id.isEmpty) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Erreur: L\'ID de l\'employé est vide. Veuillez réessayer.'),
    ...
    if (selectedEmployee.id == selectedEmployee.organizationId) {
      debugPrint('[EmployeeSelection] ❌ ERROR: Employee ID matches Organization ID!');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Erreur: L\'ID de l\'employé est incorrect. Veuillez réessayer.'),
            backgroundColor: Colors.red,
          ),
        );
      }
    ...
    try {
      debugPrint('[EmployeeSelection] 🔍 Re-fetching employee from DB to ensure correct ID...');
      
      Employee? dbEmployee;
      
      // If ID matches organization_id, we know it's wrong - search by name instead
      if (selectedEmployee.id == selectedEmployee.organizationId) {
        debugPrint('[EmployeeSelection] ⚠️ ID matches organization_id - searching by name instead...');
        final allEmployees = await _employeeRepo.getAll(activeOnly: true);
        try {
          dbEmployee = allEmployees.firstWhere(
            (emp) => emp.firstName.toLowerCase() == selectedEmployee.firstName.toLowerCase() 
                  && emp.lastName.toLowerCase() == selectedEmployee.lastName.toLowerCase(),
          );
          debugPrint('[EmployeeSelection] ✅ Found employee by name: ${dbEmployee.fullName} (ID: ${dbEmployee.id})');
        } catch (e) {
          debugPrint('[EmployeeSelection] ❌ Could not find employee by name: $e');
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('Erreur: L\'employé "${selectedEmployee.fullName}" n\'a pas pu être trouvé dans la base de données.'),
                backgroundColor: Colors.red,
        ...
        dbEmployee = await _employeeRepo.getById(selectedEmployee.id, checkCreatedBy: false);
        
        // If not found by ID, try to find by name and organization
        if (dbEmployee == null) {
          debugPrint('[EmployeeSelection] ⚠️ Employee not found by ID, searching by name and organization...');
          final allEmployees = await _employeeRepo.getAll(activeOnly: true);
          try {
            dbEmployee = allEmployees.firstWhere(
              (emp) => emp.firstName.toLowerCase() == selectedEmployee.firstName.toLowerCase() 
      ...
      if (!employeeExists) {
        debugPrint('[EmployeeSelection] ❌ Employee ID ${dbEmployee.id} does not exist in employees table!');
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Erreur: L\'employé "${dbEmployee.fullName}" n\'existe pas dans la base de données. Veuillez le recréer.'),
              backgroundColor: Colors.red,
              duration: const Duration(seconds: 5),
            ),
          );
      ...
      if (dbEmployee.id != selectedEmployee.id) {
        debugPrint('[EmployeeSelection] ✅ Found employee in DB with different ID. Old: ${selectedEmployee.id}, New: ${dbEmployee.id}');
      } else {
        debugPrint('[EmployeeSelection] ✅ Employee ID ${dbEmployee.id} verified in database');
      }
    ...
    if (selectedEmployee.isAdmin) {
      final result = await Navigator.push<bool>(
        context,
        MaterialPageRoute(
          builder: (context) => AdminCodePageWithEmployee(employee: selectedEmployee),
        ),
      );
      ...
      if (result == true && mounted) {
        ...
        debugPrint('[EmployeeSelection] ✅ Employee set: ${selectedEmployee.fullName} (ID: ${selectedEmployee.id})');
    ...
    try {
      ...
      if (openSession == null && mounted) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Vous avez une session de pointage ouverte. Vous pouvez pointer la sortie depuis la page Pointage.'),
