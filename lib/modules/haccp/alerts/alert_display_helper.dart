  ...
  String get label {
    switch (this) {
      case AlertCategory.temperature:
        return 'Température';
      case AlertCategory.reception:
        return 'Réception';
      case AlertCategory.huile:
        return 'Huile';
      case AlertCategory.nettoyage:
        return 'Nettoyage';
...
class AlertDisplayHelper {
  ...
  static AlertCategory getCategory(Alert alert) {
    switch (alert.module.toLowerCase()) {
      case 'temperature':
        return AlertCategory.temperature;
      case 'reception':
        return AlertCategory.reception;
      case 'oil':
        return AlertCategory.huile;
      case 'cleaning':
        return AlertCategory.nettoyage;
      case 'documents':
        return AlertCategory.documents;
      default:
        return AlertCategory.temperature;
    }
  }
  ...
  static String getShortTitle(Alert alert) {
    ...
    if (code.startsWith('TEMP.')) {
      if (code.contains('CRITICAL') || code.contains('CONSECUTIVE'))
    ...
    if (code.startsWith('RECEPT.')) {
      if (code.contains('CRITICAL') ||
          code.contains('SEAL') ||
          code.contains('TEMP_'))
        return 'Réception non conforme';
      if (code.contains('WET') || code.contains('PACKAGING'))
        return 'Problème emballage';
      return 'Étiquette incomplète';
    }
    if (code.startsWith('OIL.')) {
      if (code.contains('CRITICAL') || code.contains('QUALITY'))
        return 'Huile à changer immédiatement';
      return 'Huile à changer';
    }
    if (code.startsWith('CLEAN.')) {
      if (code.contains('MULTIPLE') || code.contains('CRITICAL'))
        return 'Nettoyages manqués';
