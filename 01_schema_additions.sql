-- ============================================
-- ADDITIVE SCHEMA CHANGES FOR REFONTE
-- ============================================
-- These tables extend the existing schema
...
BEGIN
  ...
  -- Legacy fournisseur_id column for compatibility
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'receptions' AND column_name = 'fournisseur_id'
  ) THEN
    ALTER TABLE public.receptions ADD COLUMN fournisseur_id UUID REFERENCES public.suppliers(id) ON DELETE SET NULL;
  END IF;
END $$;
